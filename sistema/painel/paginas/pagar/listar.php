...
$status = '%' . @$_POST['status'] . '%'; //busca de forma aproximada, quando o campo vir vazio (sem "sim" ou "não"), busca todas as contas, pagas "sim" e pagas "não"

$total_pago = 0;
$total_a_pagar = 0;

$query = $pdo->query("SELECT * FROM $tabela where data_venc >= '$dataInicial' and data_venc <= '$dataFinal' and pago LIKE '$status' ORDER BY pago asc, data_venc asc"); //pago asc começa com "Não", pois "Não" vem antes de "Sim", e depois por data de vencimento da menor para a maior
...
HTML;
	...
	for ($i = 0; $i < $total_reg; $i++) {
		...
		if ($data_venc < $data_hoje and $pago != 'Sim') { //conta atrasada não paga
...
<td>
		<big><a href="#" onclick="editar('{$id}','{$descricao}', '{$fornecedor}', '{$valor}', '{$data_venc}', '{$data_pgto}', '{$tumb_arquivo}', '{$funcionario}')" title="Editar Dados"><i class="fa fa-edit text-primary"></i></a></big>
